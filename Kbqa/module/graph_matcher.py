...
class GraphMatcher:
    ...
    def __init__(self):
        self.graph = Graph('http://localhost:7474/finance_demo/db/', auth=('neo4j', 'neo4j123'),name='neo4j')
            ...
            if each_ques_type == 'concept':
                ...
                for entity_name, entity_type in entities.items():
                    ...
                    # 2、问概念有哪些股票
                        ...
                        rtn = self.graph.run(cypher_sql).data()
                        response += f'{entity_name}概念下有{rtn[1]["s.股票名称"]}等股票' + '\n'
                ...
                for entity_name, entity_type in entities.items():
                    ...
                    if entity_type == '股票':
                        ...
                        rtn = self.graph.run(cypher_sql).data()
                        response += f'{entity_name}的最大股东是{rtn[0]["s.股东名称"]}，持有股份{rtn[0]["r.持有量"]}，占比{rtn[0]["r.占比"]}%' + '\n'
                    # 2、问股东的股票
                    elif entity_type == '股东':
